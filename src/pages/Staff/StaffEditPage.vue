<template>
  <PageTitle pageTitle="Edit Staff" subTitle="Staff" />
  ...
  <div v-if="isLoading" class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
    <div class="flex flex-col items-center justify-center py-20">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400 mt-4">Loading staff...</p>
    </div>
  </div>
      ...
      <div class="space-y-5">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input
              ...
              type="date"
              required
              class="bg-gray-50 border border-gray-300 h-[44px] text-sm rounded-md w-full block text-black px-4 outline-0 dark:bg-[#15203c] dark:text-white dark:border-[#15203c] focus:border-primary-500 dark:focus:border-primary-500"
              :class="{'border-danger-500 dark:border-danger-500': errors.dob}"
            />
            <p v-if="errors.dob" class="text-xs text-danger-500 mt-1">{{ errors.dob }}</p>
          </div>
          ...
          <div>
            ...
            <select
              ...
              required
              class="bg-gray-50 border border-gray-300 h-[44px] text-sm rounded-md w-full block text-black px-4 outline-0 dark:bg-[#15203c] dark:text-white dark:border-[#15203c] focus:border-primary-500 dark:focus:border-primary-500 cursor-pointer"
              :class="{'border-danger-500 dark:border-danger-500': errors.gender}"
            >
              <option value="">Select Gender</option>
              <option value="male">Male</option>
              <option value="female">Female</option>
            </select>
            <p v-if="errors.gender" class="text-xs text-danger-500 mt-1">{{ errors.gender }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="bg-gray-50 border border-gray-300 h-[44px] text-sm rounded-md w-full block text-black px-4 outline-0 dark:bg-[#15203c] dark:text-white dark:border-[#15203c] placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:border-primary-500 dark:focus:border-primary-500"
              :class="{'border-danger-500 dark:border-danger-500': errors.position}"
            />
            <p v-if="errors.position" class="text-xs text-danger-500 mt-1">{{ errors.position }}</p>
          </div>
        ...
        <div v-if="submitError" class="bg-danger-50 dark:bg-danger-500/10 border border-danger-500 rounded-md p-4">
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-sm font-medium text-danger-500">Failed to update staff</p>
...
// State
const staffId = ref<string>(route.params.staffId as string);
const formData = ref<StaffFormData | null>(null);
...
const fetchStaff = async () => {
  if (!staffId.value) {
    loadError.value = 'Staff ID is missing';
    return;
  }
  ...
  try {
    const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/staff/${staffId.value}`);
    ...
    if (err.response?.status === 404) {
      loadError.value = 'Staff not found';
    } else if (err.response?.status === 400) {
      loadError.value = 'Invalid staff ID';
    } else {
      loadError.value = 'Failed to load staff. Please try again later.';
    }
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await axios.patch(
      `${import.meta.env.VITE_API_URL}/staff/${staffId.value}`,
      payload
    );
    ...
    setTimeout(() => {
      ...
      router.push(`/staff/${staffId.value}`);
    }, 2000);
  } catch (err: any) {
    ...
    if (err.response?.data?.message) {
      ...
      submitError.value = 'Failed to update staff. Please try again.';
    }
  } finally {
